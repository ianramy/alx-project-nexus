...
import { fetchActionTemplate, markActionDone } from "@/utils/actions";
import { Action } from "@/interfaces/action";
import BackButton from "@/components/common/BackButton";

type ShareCapableNavigator = Navigator & { share?: (data: ShareData) => Promise<void> };

type ActionWithChallenge = Action & {
	challenge?: { id?: number | null } | null;
};
...
export default function ActionDetailPage() {
	...
	const [action, setAction] = useState<ActionWithChallenge | null>(null);
	...
	useEffect(() => {
		...
		(async () => {
			try {
				const found = await fetchActionTemplate(idNum);
				if (mounted) setAction(found as ActionWithChallenge);
			} finally {
	...
	const share = async () => {
		try {
			...
			if (nav?.share) {
				await nav.share({ title: action?.action_type, text: action?.description, url });
	...
	const onMarkDone = async () => {
		if (!action || submitting || completed) return;
		setSubmitting(true);
		try {
			const challengeId = action.challenge?.id ?? null;
			await markActionDone({
			...
			alert("Couldn’t mark as done. Are you logged in?");
		} finally {
			setSubmitting(false);
		}
	};
									...
									<div className="rounded-2xl bg-white/70 dark:bg-slate-800/70 ring-1 ring-black/5 p-4">
										...
										<div className="mt-4 grid gap-2 text-sm">
											...
											<div className="flex items-center justify-between">
												...
												<span className={`rounded-full px-2 py-0.5 text-xs ring-1 ${completed
													? "bg-emerald-200 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-200 ring-emerald-600/20"
													: "bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-200 ring-emerald-600/20"
												}`}>
										...
										<button
											...
											className={`
												...
												${completed ? "bg-emerald-700" : "bg-emerald-500 hover:bg-emerald-600 active:bg-emerald-700"}
												text-white font-semibold px-4 py-2.5 transition disabled:opacity-60
											`}
											onClick={onMarkDone}
										>
											{completed ? "Completed ✔" : (submitting ? "Saving..." : "Mark as done")}
										</button>
									</div>
								</aside>
							</article>
